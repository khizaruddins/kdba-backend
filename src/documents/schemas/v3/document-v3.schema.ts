...
export const PageDocumentV3Schema = z.object({
  ...
  slug: z
    ...
    .refine(
      (val) => /^(\/|\/[a-zA-Z0-9-_/]*|\/[a-zA-Z0-9-_/]*:slug)$/.test(val),
      {
        message: 'Invalid page slug format (must begin with /; dynamic item pages may end with :slug)',
      },
    ),
  type: z
    .string()
...
export const DocumentOperationSchema = z.discriminatedUnion('type', [
  ...
  z.object({
    ...
    pageId: safeString(100),
    nodeId: safeString(100),
    breakpoint: safeString(50).optional(),
  ...
  z.object({
    ...
    pageId: safeString(100),
    targetParentId: safeString(100).optional(),
    parentId: safeString(100).optional(),
    node: WebsiteNodeSchema,
    index: z.number().int().min(0).optional(),
  }),
  z.object({
